...
import org.nabucco.testautomation.script.ui.rcp.multipage.metadata.wizard.find.FindAndReplaceWizard;
...
public class FindAndReplaceHandlerImpl implements FindAndReplaceHandler {
	...
	/* (non-Javadoc)
	 * @see org.nabucco.testautomation.script.ui.rcp.command.metadata.FindAndReplaceHandler#findAndReplace()
	 */
	@Override
	public void findAndReplace() {
		MetadataMaintenanceMultiPageEditViewModel model = (MetadataMaintenanceMultiPageEditViewModel)Activator.getDefault().getModel().getCurrentView().getModel();
		int matches = 0;
		if(node != null) {
	...
	private int findAndReplace(Datatype datatype) {
		if(datatype != null) {
			if(datatype instanceof Metadata) {
				...
				wizard = new FindAndReplaceWizard(metadata);
				...
				MetadataLabel label = (MetadataLabel) datatype;
				return this.findAndReplace(label.getPropertyList());
			}
			return startWizard();
		}
		return 0;
	...
	/* (non-Javadoc)
	 * @see org.nabucco.testautomation.script.ui.rcp.command.metadata.FindAndReplaceHandler#setNode(org.nabucco.framework.plugin.base.component.multipage.masterdetail.MasterDetailTreeNode)
	 */
	@Override
