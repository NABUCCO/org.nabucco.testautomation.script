	...
	private Map.Entry<String, Map<String, ? extends Serializable>> getText(final Datatype datatype) {
		...
		if (datatype instanceof Folder) {
			...
			if (folder.getName() != null) {
				result = createEntry("MasterDetailTree.Folder", "name", folder.getName()
						.getValue());
			} else {
				result = createEntry("MasterDetailTree.Folder", "name", folder.getClass()
						.getSimpleName());
			}
		} else if (datatype instanceof TestScript) {
			TestScript testScript = (TestScript) datatype;
			if (testScript.getName() != null) {
				result = createEntry("MasterDetailTree.TestScript", "name", testScript.getName()
						.getValue());
			} else {
				result = createEntry("MasterDetailTree.TestScript", "name", testScript.getClass()
						.getSimpleName());
			}
		}
		return result;
