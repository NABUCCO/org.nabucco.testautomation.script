...
public class TestScriptSearchViewLayouter implements NabuccoLayouter<TestScriptSearchViewModel> {
    ...
    @Override
    public Composite layout(Composite parent, NabuccoMessageManager messageManager, TestScriptSearchViewModel model,
            Layoutable<TestScriptSearchViewModel> view) {
        return layout(parent, messageManager, model);
    }

    @Override
    public Composite layout(Composite parent, final NabuccoMessageManager messageManager,
            TestScriptSearchViewModel model) {
        NabuccoFormToolkit ntk = new NabuccoFormToolkit(parent);
        TestScriptSearchViewWidgetFactory widgetFactory = new TestScriptSearchViewWidgetFactory(ntk, model);
        ...
        layoutElementKey(widgetFactory, child);
     ...
     *            the parent composite
     */
    private void layoutElementKey(TestScriptSearchViewWidgetFactory widgetFactory, Composite parent) {
        Label label = widgetFactory.createLabelTestScriptKey(parent);
        this.layout(label);

        Text text = widgetFactory.createInputFieldTestScriptKey(parent);
        this.layout(text);
    }
