...
public class MetadataSearchViewLayouter implements NabuccoLayouter<MetadataSearchViewModel> {
    ...
    private final static String TEST_SCRIPT_TEXT = MetadataSearchView.ID + ".section";

    private final static String SUB_ENGINE = MetadataSearchView.ID + ".subEngine";

    private MetadataSearchViewModel model;

    @Override
    public Composite layout(Composite parent, NabuccoMessageManager messageManager, MetadataSearchViewModel model,
            Layoutable<MetadataSearchViewModel> view) {
        return layout(parent, messageManager, model);
    }

    @Override
    public Composite layout(Composite parent, final NabuccoMessageManager messageManager, MetadataSearchViewModel model) {
        this.model = model;

        NabuccoFormToolkit ntk = new NabuccoFormToolkit(parent);
        MetadataSearchViewWidgetFactory widgetFactory = new MetadataSearchViewWidgetFactory(ntk, model);

        // build a section to host the other controls
        Section section = ntk.createSection(parent, TEST_SCRIPT_TEXT, new RowLayout());
     ...
     */
    private void layoutName(MetadataSearchViewWidgetFactory widgetFactory, Composite parent) {
     ...
     *            the parent composite
     */
    private void layoutDescription(MetadataSearchViewWidgetFactory widgetFactory, Composite parent) {
        Label label = widgetFactory.createLabelMetadataKey(parent);
        this.layout(label);

        Text text = widgetFactory.createInputFieldMetadataKey(parent);
        this.layout(text);
    }
    ...
    private void layoutSubEngineCode(NabuccoFormToolkit nabuccoFormToolkit, Composite child) {

        Label label = nabuccoFormToolkit.createRealLabel(child, SUB_ENGINE);
        this.layout(label);

        final List<SubEngineCode> subEngineCodes = MetadataElementFactory.getSubEngineCodes(null);
        ObjectListContentProvider subEngineCodeContentProvider = new ObjectListContentProvider(subEngineCodes);
        ...
        Control newWidget = nabuccoFormToolkit.createElementPickerCombo(child, subEngineCodeParameter, false, true);
        ((ElementPickerCombo) newWidget).addSelectionListener(new SelectionAdapter() {

            @Override
            public void widgetSelected(SelectionEvent arg0) {
                String selectedText = ((Combo) arg0.getSource()).getText();
                for (SubEngineCode subEngineCode : subEngineCodes) {
                    String subEngineCodeName = subEngineCode.getName().toString();
                    if (subEngineCodeName.equals(selectedText)) {
                        MetadataSearchViewLayouter.this.model.getMetadata().setSubEngine(subEngineCode);
                    } else if (selectedText.isEmpty()) {
                        MetadataSearchViewLayouter.this.model.getMetadata().setSubEngine(null);
                    }
                }
            }
