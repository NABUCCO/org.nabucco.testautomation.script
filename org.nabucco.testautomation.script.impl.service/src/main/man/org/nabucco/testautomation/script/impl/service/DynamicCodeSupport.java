...
public class DynamicCodeSupport {
	...
	public void resolveDynamicCodes(MetadataLabel label, ServiceMessageContext ctx) throws SearchException {
	    try {
	    	Long brandId = label.getBrandTypeRefId();
	    	Long releaseId = label.getReleaseTypeRefId();
	    	Long envId = label.getEnvironmentTypeRefId();
	    	
	    	if (brandId != null) {
	    		Code code = searchDynamicCode(brandId, ctx);
	    		label.setBrandType(code);
	    	}
	    	if (releaseId != null) {
	    		Code code = searchDynamicCode(releaseId, ctx);
	    		label.setReleaseType(code);
	    	}
	    	if (envId != null) {
	    		Code code = searchDynamicCode(envId, ctx);
	    		label.setEnvironmentType(code);
	    	}
	    } catch (Exception e) {
